...
exports.openMusic = function (options) {
  ...
  var testName = options.testName || 'Generic test';
  ...
  before(function handleLogin (done) {
    ...
    async.waterfall([
      ...
      function enterEmailIntoInput (el, cb) {
        exports.simpleExecute.call(that, function setEmailInputValue () {
          document.getElementById('Email').value = arguments[0];
        }, [GOOGLE_MUSIC_JS_EMAIL], cb);
      },
      function findNextButton (executeResult, cb) {
      ...
      function enterPasswordIntoInput (el, cb) {
        exports.simpleExecute.call(that, function setPasswordInputValue () {
          document.getElementById('Passwd').value = arguments[0];
        }, [GOOGLE_MUSIC_JS_PASSWORD], cb);
      },
      function findLoginButton (executeResult, cb) {
        browser.waitForElementById('signIn', asserters.isDisplayed, cb);
      },
      function clickLoginButton (el, cb) {
        exports.simpleExecute.call(that, function clickSignInButton () {
...
exports.simpleExecute = function (fn, args, cb) {
  // DEV: Barebones execute function on the browser, this works when .execute does not
  this.browser.execute(functionToString(fn).body, args, cb);
};
